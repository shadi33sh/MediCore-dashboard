...
import { FaAlignLeft, FaArrowLeft, FaChevronDown, FaChevronLeft, FaChevronRight } from 'react-icons/fa'
...
const [actionLoading, setActionLoading] = useState({})
  ...
  const transformApiData = (apiData) => {
    return apiData.map(item => ({
      ...
      patient: {
        ...
        medicalStatus: item.patient.chronic_diseases !== 'non' ? item.patient.chronic_diseases : 'Stable'
      },
  ...
  const sections = ['All', ...new Set(scheduleList.map(item => item.doctor.section))]
  ...
  const filteredSchedules = sortAppointmentsByDateTime(
    scheduleList.filter(item => {
      ...
      const searchMatch = item.patient.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const acceptedSchedules = filteredSchedules.filter(item => item.status === 'accepted')
  const waitingSchedules = filteredSchedules.filter(item => item.status === 'waiting')
  ...
  const handleStatusChange = async (itemIndex, newStatus) => {
    ...
    try {
      ...
      // Update local state
      const indexInList = scheduleList.findIndex(schedule => schedule.id === appointmentId)
      if (indexInList !== -1) {
  ...
  useEffect(() => {
    const getAppointments = async () => {
      try {
        ...
        const response = await axiosInstance.get('api/secretary/apointments')
      ...
      <div className="mb-6">
        ...
        >
          <div className="flex items-center space-x-3">
            {openTables[type] ? <FaChevronDown className="text-Primary" /> : <FaChevronRight className="text-Primary" />}
                      ...
                      return (
                        <React.Fragment key={uniqueKey}>
                          <tr
                            ...
                            `}
                          >
                            <td onClick={() => toggleDetails(uniqueKey)}  className="px-4 py-6 text-sm font-medium text-gray-800 dark:text-gray-200">
                            ...
                            <td onClick={() => toggleDetails(uniqueKey)} className="px-4 py-6 text-sm font-bold  ">
                              <div className="flex items-center space-x-2">
                                <div className="w-2 h-2 rounded-full bg-Primary "></div>
                                <span>{item.doctor.name}</span>
                              </div>
                            </td>
                            <td onClick={() => toggleDetails(uniqueKey)} className="px-4 py-6 text-sm text-gray-600 dark:text-gray-400">
                              <div className="flex items-center space-x-2">
                                    ...
                                    <div className="p-8 flex flex-wrap gap-8 bg-gradient-to-r from-gray-50 to-blue-50 dark:from-gray-800/30 dark:to-gray-700/30 border-t border-gray-200 dark:border-gray-700/5">
                                      ...
                                      >
                                        ...
                                        <img
                                            ...
                                            onError={(e) => {
                                              e.target.src = "/images/default-doctor.jpg" // Replace with your default image path
                                            }}
                                          />
                                          <div className="absolute bottom-3 right-3 w-5 h-5 bg-gradient-to-r from-Primary to-blue-500 rounded-full flex items-center justify-center">
                                          </div>
                                        </div>
                                        <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-1">{item.doctor.name}</h3>
                                        <p className="text-Primary dark:text-Primary mb-2 font-medium">{item.doctor.section}</p>
                                        <div className="flex items-center space-x-1">
                                          <span className="text-yellow-400 text-lg">⭐</span>
                                          <span className="text-yellow-600 dark:text-yellow-400 font-bold">{item.doctor.rating}</span>
                                        </div>
                                      </motion.div>
                                      ...
                                      >
                                        <div className="flex items-center space-x-2 mb-4">
                                          ...
                                          <h3 className="text-xl font-bold text-gray-800 dark:text-white">Patient Info</h3>
                                        </div>
                                        <div className="space-y-3">
                                          <p className="flex items-center space-x-2">
                                            <span className="font-bold text-Primary dark:text-Primary ">Name:</span>
                                            <span className="text-gray-700 dark:text-gray-300">{item.patient.name}</span>
                                          </p>
                                          <p className="flex items-center space-x-2">
                                            <span className="font-bold text-Primary dark:text-Primary ">Age:</span>
                                            <span className="text-gray-700 dark:text-gray-300">{item.patient.age}</span>
                                          </p>
                                          <p className="flex items-center space-x-2">
                                            <span className="font-bold text-Primary dark:text-Primary ">Gender:</span>
                                            <span className="text-gray-700 dark:text-gray-300">{item.patient.gender}</span>
                                          </p>
                                          <p className="flex items-center space-x-2">
                                            <span className="font-bold text-Primary dark:text-Primary ">Medical Status:</span>
                                            <span className="text-gray-700 dark:text-gray-300">{item.patient.medicalStatus}</span>
                                          </p>
                                          <p className="flex items-center space-x-2">
                                            <span className="font-bold text-Primary dark:text-Primary ">Medical Status:</span>
                                            <span className="text-gray-700 dark:text-gray-300">{item.patient.medicalStatus}</span>
  ...
  return (
    <DashboardLayout title='Doctor Appointments' loading={loading}>
