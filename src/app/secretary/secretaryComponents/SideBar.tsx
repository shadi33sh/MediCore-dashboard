...
import { IoDocument, IoSettings, IoTabletLandscape, IoTabletPortrait, IoNotifications } from "react-icons/io5";
import { medicalSections } from '../sections/page'
import { motion, AnimatePresence } from 'framer-motion'
import ToggleModeButton from '../../../Components/ToggleModeButton'
import AddMounthlyWorkDays from './AddmounthlyLeave'

const sidebarItems = [
  { label: 'Dashboard', icon: <FaHome size={18} />, href: '/secretary', badge: null },
  // { label: 'Statistics', icon: <FaCalendarAlt size={18} />, href: '/secretary/statistics', badge: null },
  { label: 'Schedules', icon: <FaCalendarAlt size={18} />, href: '/secretary/schedules', badge: '3' },
  { label: 'Doctors', icon: <FaUserInjured size={18} />, href: '/secretary/doctors', badge: null },
        ...
        <div className="h-full flex flex-col">
          ...
          <div className="flex items-center justify-between p-6">
            ...
            <div className="flex items-center gap-2">
              {!isMobile && (
                ...
                >
                  {collapsed ? 
                    <FaChevronRight size={16} className="text-gray-600 dark:text-gray-400" /> : 
                    <FaChevronLeft size={16} className="text-gray-600 dark:text-gray-400" />
                  }
                </motion.button>
              )}
              ...
              {isMobile && (
                <motion.button
                  ...
                  className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
                >
                  <FaTimes size={16} className="text-gray-600 dark:text-gray-400" />
                </motion.button>
              )}
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-4 pb-4">
            <div className="space-y-2">
              {(searchQuery ? filteredItems : sidebarItems).map((item, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <span className={`${isActive ? 'text-white' : 'text-gray-500 dark:text-gray-400 group-hover:text-Primary dark:group-hover:text-blue-400'} transition-colors`}>
                        {item.icon}
                      </span>
              ...
              >
                <FaBoxes size={18} className="group-hover:text-white transition-colors" />
                  ...
                  <div className="space-y-6">
                    {settingsTab === 'profile' && (
                      <div className="space-y-4">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <h4 className="font-semibold text-gray-800 dark:text-white">{user.first_name} {user.last_name}</h4>
                            <p className="text-gray-600 dark:text-gray-400">{user.role}</p>
                          </div>
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                          <input type="text" placeholder="First Name" className="p-3 border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800" />
                          <input type="text" placeholder="Last Name" className="p-3 border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800" />
                          <input type="email" placeholder="Email" className="p-3 border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 col-span-2" />
                        </div>
                      </div>
                    )}
                    ...
                    {settingsTab === 'appearance' && (
                      <div className="space-y-4">
                        ...
                        <div className="flex items-center justify-between">
                          <span className="text-gray-700 dark:text-gray-300">Compact Sidebar</span>
                          <button 
                            onClick={() => setCollapsed(!collapsed)}
                            className={`w-12 h-6 rounded-full transition-colors ${collapsed ? 'bg-Primary' : 'bg-gray-300 dark:bg-gray-600'}`}
                          >
                            <div className={`w-5 h-5 bg-white rounded-full shadow-md transform transition-transform ${collapsed ? 'translate-x-6' : 'translate-x-0.5'}`} />
                          </button>
                        </div>
                      </div>
                    )}
        ...
        {showReserveForm && (
          <>
            ...
            <motion.div
              ...
              >
              <button onClick={() => setReserve(false)} className="absolute top-3 right-4 text-xl hover:text-red-400 transition"> &times;</button>
               <AddMounthlyWorkDays/>
            </motion.div>
          </>
        )}
