...
export default function NewAppointmentModal({
  ...
  const [selectedAppointmentDate, setAppointmentDate] = useState<any>(initialDate)
  const [selectedDepartmentID, setDepartmentID] = useState<any>(initialDepartment)
  const [filteredTable, setFilteredTable] = useState<any[]>([])

  const [doctors, setDoctors] = useState<any[]>([])
  const [departments, setDepartments] = useState<any[]>([])
  const [scheduleTable, setScheduleTable] = useState<any[]>([])
  const [doctorAppiontments, setDoctorAppiontments] = useState<any>(null)
  const [selectedAppointmentFinelTime, setAppointmentFinalTime] = useState<any>(initialDate)
  ...
  const bookedSlots = React.useMemo(() => {
    ...
    return doctorAppiontments.map((app: any) => {
      const dateStr = app.apointment_date || ''
  ...
  const getDayId = (jsDay: number) => {
    switch (jsDay) {
      ...
      case 6: return 7; default: return null
    }
  }
  ...
  const generateMonthDays = () => {
    ...
    for (let empty = 0; empty < firstDayOfMonth; empty++) {
      days.push({ date: null, doctors: [], clickable: false, placeholder: true })
    }
    for (let i = 1; i <= daysInMonth; i++) {
      const date = dayjs(new Date(year, month, i))
      const day_id = getDayId(date.day())
      const doctorsForThatDay = filteredTable.filter((entry: any) => entry.day_id === day_id)
      const uniqueDoctorsForThatDay = doctorsForThatDay.filter((v: any, i: number, a: any[]) => a.findIndex((t: any) => t.doctor_id === v.doctor_id) === i)
      days.push({
        date: date.format('YYYY-MM-DD'),
        doctors: uniqueDoctorsForThatDay,
        clickable: uniqueDoctorsForThatDay.length > 0,
        placeholder: false,
      })
    }
    return days
    ...
    <AnimatePresence>
      {open && (
        <>
          {/* Backdrop */}
          <motion.div
            className="fixed -inset-20 bg-[#02354e85] backdrop-blur-sm z-40 flex items-center justify-center p-4"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            onClick={onClose}
          >
            {/* Modal panel */}
            <motion.div
              className="relative w-[80vw] max-h-[90vh] bg-white dark:bg-gray-900 rounded-3xl shadow-2xl overflow-hidden flex flex-col"
              ...
              <div className="overflow-y-auto flex-1 px-6 py-5">
                <form className="space-y-5">
                  ...
                  <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm p-5">
                    ...
                    {!initialPatientID && isPatientInSystem && (
                    ...
                    <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm p-5">
                      ...
                      <div className="grid grid-cols-7 gap-2">
                        {generateMonthDays().map((day, index) => {
                          if (day.placeholder) {
                            return <div key={`empty-${index}`} className="p-2 rounded-xl text-sm shadow-sm transition-all text-center bg-transparent" />
                          }
          ...
          <AnimatePresence>
            {isModal && (
              <motion.div
                className="fixed flex items-center justify-center inset-0 bg-black/60 backdrop-blur-sm z-50"
              ...
              >
                <motion.div
                  className="z-50 w-[92vw] max-w-lg bg-white dark:bg-gray-900 rounded-3xl shadow-2xl overflow-hidden"
