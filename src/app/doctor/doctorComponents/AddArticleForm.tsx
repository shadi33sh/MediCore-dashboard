...
export default function AddArticleForm({ onArticleAdded }: AddArticleFormProps) {
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      showAlert("success", t('Doctor.ArticlesPage.publishSuccess', 'Article published successfully!'));
      setFormData({ title: "", content: "" });
      if (onArticleAdded && response.data.data) {
        onArticleAdded(response.data.data);
      }
      return response.data.data;
    } catch (error: any) {
      showAlert("error", error.response?.data?.message || t('Doctor.ArticlesPage.publishFail', 'Failed to upload article'));
      return null;
    } finally {
      setLoading(false);
    }
    ...
    >
      ...
      <div className="p-5 border-b border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900">
        <label className="text-xs font-bold text-gray-400 uppercase tracking-wide flex items-center gap-1.5 mb-2">
          ...
          {t('Doctor.ArticlesPage.articleTitle', 'Article Title')}
        </label>
        <input
          type="text"
          name="title"
          ...
          placeholder={t('Doctor.ArticlesPage.titlePlaceholder', 'e.g. 10 Tips to Keep Your Heart Healthy')}
      ...
      <div className="flex-1 overflow-y-auto p-5 bg-white dark:bg-gray-900 min-h-[300px]">
        <label className="text-xs font-bold text-gray-400 uppercase tracking-wide block mb-2">
          {t('Doctor.ArticlesPage.content', 'Content')}
        </label>
      ...
      <div className="p-5 border-t border-gray-150 dark:border-gray-800 bg-white dark:bg-gray-900 flex justify-end">
        {loading ? (
          ...
          >
            <FiCheck size={16} />
            {t('Doctor.ArticlesPage.publishBtn', 'Publish Article')}
          </button>
        )}
      </div>
    </form>
